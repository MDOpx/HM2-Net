...
class LinearProjection(nn.Module):
    ...
    def forward(self, x, attn_kv=None):
        ...
        # print('B_, N, 1, self.heads, C // self.heads:', B_, N, 1, self.heads, C // self.heads)
...
class LinearProjection_replace(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0., bias=True):
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size,num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        if token_projection =='linear':
            ...
            # self.qkv = LinearProjection(dim, num_heads, 16, bias=qkv_bias)
            # print(dim,num_heads)
        else:
...
########### window-based self-attention #############
class WindowAttention_sparse_replace(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0.,
        ...
        if token_projection == 'linear':
            self.qkv = LinearProjection_replace(dim, num_heads, dim // num_heads, bias=qkv_bias)
        else:
            raise Exception("Projection error!")
        ...
        self.map_reshape = Map_reshape([4, 4], self.num_heads)
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        # print('relative_position_bias.shape:', relative_position_bias.unsqueeze(0).shape)
        attn = attn + relative_position_bias.unsqueeze(0)
...
class WindowAttention_sparse(nn.Module):
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        #     input_mask_windows = window_partition(mask, self.win_size[0]) # nW, win_size, win_size, 1
        #     attn_mask = input_mask_windows.view(-1, self.win_size[0] * self.win_size[0]) # nW, win_size*win_size
        #     attn_mask = attn_mask.unsqueeze(2)*attn_mask.unsqueeze(1) # nW, win_size*win_size, win_size*win_size
        #     mask = attn_mask.masked_fill(attn_mask!=0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
        ...
        # map_attn = self.map_reshape(attn)

        # print('map_attn.shape',map_attn.shape)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=4, shift_size=0,
    ...
    def forward(self, x, mask=None):
        ...
        if mask != None:
            ...
            print('input_mask_windows.shape',input_mask_windows.shape)
            attn_mask = input_mask_windows.view(-1, self.win_size * self.win_size) # nW, win_size*win_size
            print('attn_mask0.shape', attn_mask.shape)
            # attn_mask = attn_mask.permute(1,0)
            attn_mask = attn_mask.unsqueeze(2)*attn_mask.unsqueeze(1) # nW, win_size*win_size, win_size*win_size
            print('attn_mask1.shape',attn_mask.shape)
        ...
        if self.shift_size > 0:
            ...
            print('attn_mask2.shape', attn_mask.shape)
        ...
        if self.att:
            ...
            # print('attn_mask3.shape:', attn_mask.shape)
...
class assa(nn.Module):
    ...
    def forward(self, x, mask=None,mask2 = None):
        ...
        # k = self.pool(self.key(x)).view(b, -1, int(h * w/4))

        q = self.query(x)
        k = self.pool(self.key(x))
        # print(k.shape)
        # mask_pool = mask.squeeze(1)
        ...
        k = k.view(b, -1, int(h * w/4))

        v = self.pool(self.value(x)).view(b, -1, int(h * w/4))
        ...
        print('torch.unique(out)',torch.unique(out))
        # print(out.shape)
