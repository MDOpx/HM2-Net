...
import numpy as np
# from networks.uctnet_2D import DeepSupervision
import cv2
...
class Attention(nn.Module):
    ...
    def forward(self, x, map_attn, add_Map,casename):
        # print(max_point)
        ...
        # print('map_attn0.shape', map_attn.shape)
        if add_Map and (map_attn is not None):
            attn = map_attn * attn 
        # print('attn.shape',attn.shape)
        # print('v.shape', v.shape)
        # patch_idx = 0
        # attn_map = attn[0,:,max_point,:]
        # attn_map = torch.mean(attn_map,dim = 0)
        # reshape_uct = Rearrange('(h w) -> (h) (w)', h=14)
        # attn_map_flat = reshape_uct(attn_map)
        #
        # min = torch.min(attn_map_flat)
        # max = torch.max(attn_map_flat)
        #
        # heatmap_uct = ((attn_map_flat - min)/(max-min)*255.0)
        # # print('torch.unique(heatmap_uct)',torch.unique(heatmap_uct))
        #
        # heatmap_uct = heatmap_uct.squeeze().detach().cpu().numpy().astype(np.uint8)
        # heatmap_uct[heatmap_uct > 255] = 255
        # heatmap_uct[heatmap_uct < 0] = 0
        # heatmap_uct = cv2.applyColorMap(heatmap_uct, cv2.COLORMAP_HOT)
        # cv2.imwrite('/home/shuoxing/data/TransUNet/Results-Final-MFC/attn_map/' + casename + '.png', heatmap_uct)
        # print('torch.unique(attn_map)',torch.unique(attn_map))
...
class Transformer(nn.Module):
    ...
    def forward(self, x, map_attn, add_Map, casename):
        for attn, ff in self.layers:
            ...
            # # print(len(max_point))
            # print(len(add_Map))
            att_out = attn(x,map_attn,add_Map,casename = casename)
            x = att_out + x
            x = ff(x,None,None) + x
        return x
...
class Vision_Transformer(nn.Module):
    ...
    def forward(self, x, A_map, casename):
        h, w = self.input_resolution
        B, C, H, W = x.shape
        # print(h,w)
        # print(H,W)
        ...
        # print(f"Before patch_embedding: x.requires_grad={x.requires_grad}")

        x = self.patch_embedding(x)
        # print(f"After patch_embedding: x.requires_grad={x.requires_grad}")

        x = x + self.pos_embedding
        # print(f"After adding pos_embedding: x.requires_grad={x.requires_grad}")
        ...
        x = self.transformer(x, map_attn, self.add_Map, casename)
        # print(f"After transformer: x.requires_grad={x.requires_grad}")

        vit_out = self.recover_patch_embedding(x)
        # print(f"After recover_patch_embedding: vit_out.requires_grad={vit_out.requires_grad}")
