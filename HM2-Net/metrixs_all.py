...
def evaluate_best_model(predictions_folder, label_folder, output_file, num_classes=2):
    """ 评估 `best_model_results` 结果，并保存到文件 """

    if not os.path.exists(predictions_folder):
        print(f"⚠️  预测结果文件夹不存在: {predictions_folder}")
        return

    predictions = load_png_files(predictions_folder)  # 预测结果不需要 resize
    ...
    if not common_files:
        print(f"⚠️  {predictions_folder} 无匹配的预测和标签文件，跳过...")
        return

    print(f"📊 正在计算 `{predictions_folder}` 的指标...")
    ...
    with open(output_file, 'w') as f:
        f.write(f"Evaluation Metrics for `{predictions_folder}`\n")
...
if __name__ == "__main__":
    ...
    datasets = [
        ("/home/shuoxing/data/TransUNet/Results/KAT/CVC-300",
         "/home/shuoxing/data/TransUNet/dataset/TestDataset/CVC-300/masks",
         "/home/shuoxing/data/TransUNet/Results/metrixs/KAT-MFC/best_results_CVC-300.txt"),

        ("/home/shuoxing/data/TransUNet/Results/KAT/CVC-ClinicDB",
         "/home/shuoxing/data/TransUNet/dataset/TestDataset/CVC-ClinicDB/masks",
         "/home/shuoxing/data/TransUNet/Results/metrixs/KAT-MFC/best_results_ClinicDB.txt"),

        ("/home/shuoxing/data/TransUNet/Results/KAT/CVC-ColonDB",
         "/home/shuoxing/data/TransUNet/dataset/TestDataset/CVC-ColonDB/masks",
         "/home/shuoxing/data/TransUNet/Results/metrixs/KAT-MFC/best_results_CVC-ColonDB.txt"),

        ("/home/shuoxing/data/TransUNet/Results/KAT/ETIS",
         "/home/shuoxing/data/TransUNet/dataset/TestDataset/ETIS-LaribPolypDB/masks",
         "/home/shuoxing/data/TransUNet/Results/metrixs/KAT-MFC/best_results_ETIS.txt"),

        ("/home/shuoxing/data/TransUNet/Results/KAT/Kvasir",
         "/home/shuoxing/data/TransUNet/dataset/TestDataset/Kvasir/masks",
         "/home/shuoxing/data/TransUNet/Results/metrixs/KAT-MFC/best_results_Kvasir.txt"),
    ]

    for pred_path, label_path, output_file in datasets:
        print(f"\n🔍 开始评估数据集: {pred_path}")
        evaluate_best_model(pred_path, label_path, output_file, num_classes=2)
