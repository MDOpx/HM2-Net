...
import torch
from networks.crossentropy import RobustCrossEntropyLoss
from networks.nd_softmax import softmax_helper
from networks.tensor_utilities import sum_tensor
...
class DC_and_CE_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        ce_loss = self.ce(net_output, target[:, 0].float()) if self.weight_ce != 0 else 0
        if self.ignore_label is not None:
            ce_loss *= mask[:, 0]
