...
class PVMLayer(nn.Module):
    ...
    def forward(self, x, map_attn=None, add_map=None):
        if x.dtype == torch.float16:
            x = x.type(torch.float32)
        B, C = x.shape[:2]
        # print('x.shape[:2]',x.shape)
        # print('dim',self.input_dim)
        ...
        if (map_attn is not None) and (add_map is not None):
            map_attn = map_attn.reshape(B,1,n_tokens).transpose(-1,-2)
            x_norm = (1+map_attn)*x_norm
        # if (map_attn is not None) and (add_map is not None):
        #     map_attn = map_attn.reshape(B,1,n_tokens).transpose(-1,-2)
        #     x_norm = map_attn*x_norm
...
class PVMLayer_single(nn.Module):
    def __init__(self, input_dim, output_dim, d_state=16, d_conv=4, expand=2):
        super().__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
...
class PVMLayer_bing(nn.Module):
    ...
    def forward(self, x, map_attn=None, add_map=None):
        ...
        x_mamba2 = self.mamba1(x2) + self.skip_scale * x2
        x_mamba3 = self.mamba2(x3) + self.skip_scale * x3
        x_mamba4 = self.mamba2(x4) + self.skip_scale * x4
        x_mamba = torch.cat([x_mamba1, x_mamba2, x_mamba3, x_mamba4], dim=2)
