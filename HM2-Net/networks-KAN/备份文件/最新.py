...
class EfficientSelfAttention(nn.Module):
    ...
    高效自注意力：
    1. 输入特征通过 LayerNorm
    2. 1/2C 通道划分为 4 份（每份 1/8C）
    3. 每份特征分别通过 2、4、6、8 层自注意力
    4. 自注意力输出 + 残差连接
    5. 最终拼接，BatchNorm + ReLU
    """
    def __init__(self, in_channels, num_heads=4):
        super(EfficientSelfAttention, self).__init__()
        assert in_channels % 2 == 0, "Input channels must be divisible by 2"

        self.norm = nn.LayerNorm(in_channels)  # 先进行 LN
        self.split_channels = in_channels // 4  # 每个子通道为 1/8C
        ...
        self.attn_6 = nn.MultiheadAttention(embed_dim=self.split_channels, num_heads=num_heads, batch_first=True)
        self.attn_8 = nn.MultiheadAttention(embed_dim=self.split_channels, num_heads=num_heads, batch_first=True)
    ...
    def forward(self, x):
        ...
        x = self.norm(x)  # 先进行 LN
        x = x.permute(0, 3, 1, 2).contiguous()  # (B, C, H, W)

        # **通道划分**
        x_split = torch.chunk(x, 4, dim=1)  # 拆成 4 份，每份 1/8C

        # **转换为序列格式（适用于自注意力）**
        x_seq = [f.view(B, self.split_channels, -1).permute(0, 2, 1) for f in x_split]  # (B, H*W, C//8)

        # **每份分别经过不同层数的自注意力**
        attn_out1, _ = self.attn_2(x_seq[0], x_seq[0], x_seq[0])  # 2 层注意力
        attn_out2, _ = self.attn_4(x_seq[1], x_seq[1], x_seq[1])  # 4 层注意力
        attn_out3, _ = self.attn_6(x_seq[2], x_seq[2], x_seq[2])  # 6 层注意力
        attn_out4, _ = self.attn_8(x_seq[3], x_seq[3], x_seq[3])  # 8 层注意力

        # **恢复原始形状**
        attn_out1 = attn_out1.permute(0, 2, 1).view(B, self.split_channels, H, W)
        attn_out2 = attn_out2.permute(0, 2, 1).view(B, self.split_channels, H, W)
        attn_out3 = attn_out3.permute(0, 2, 1).view(B, self.split_channels, H, W)
        attn_out4 = attn_out4.permute(0, 2, 1).view(B, self.split_channels, H, W)

        # **残差连接**
        out1 = attn_out1 + x_split[0]
        out2 = attn_out2 + x_split[1]
        out3 = attn_out3 + x_split[2]
        out4 = attn_out4 + x_split[3]

        # **拼接 + BN + ReLU**
        out = torch.cat([out1, out2, out3, out4], dim=1)  # 拼接为 1/2C
