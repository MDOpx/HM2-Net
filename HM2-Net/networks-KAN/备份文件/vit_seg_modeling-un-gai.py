...
class EnhancedSkipConnection(nn.Module):
    """
    ETU 增强跳跃连接:
    1. 采用 query-key-value 机制计算注意力。
    2. 通过全连接层学习跳跃连接特征的重要性。
    3. 结合不确定性特征图调整跳跃连接权重。
    """

    def __init__(self, in_channels):
        super(EnhancedSkipConnection, self).__init__()
        self.query_conv = nn.Conv2d(in_channels, in_channels // 2, kernel_size=1, bias=False)
        self.key_conv = nn.Conv2d(in_channels, in_channels // 2, kernel_size=1, bias=False)
        self.value_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1, bias=False)
        self.softmax = nn.Softmax(dim=-1)
        self.gamma = nn.Parameter(torch.zeros(1))  # 控制增强强度
    ...
    def forward(self, x):
        ...
        attn_scores = torch.bmm(query.permute(0, 2, 1), key)  # (B, H*W, H*W)
        attn_scores = self.softmax(attn_scores)

        enhanced_features = torch.bmm(value, attn_scores.permute(0, 2, 1))  # (B, C, H*W)
        enhanced_features = enhanced_features.view(B, C, H, W)

        return self.gamma * enhanced_features + x  # 结合原始特征


def compute_uncertainty_map(feature_map, kernel_size=3, sigma=1.0):
    """
    计算不确定性特征图，并使用高斯平滑
    :param feature_map: Tensor of shape (B, C, H, W)
    :param kernel_size: 高斯核大小
    :param sigma: 高斯平滑标准差
    :return: 平滑的不确定性图 (B, H, W)
    """
    mean = feature_map.mean(dim=1, keepdim=True)  # 计算通道均值
    uncertainty = torch.sqrt(((feature_map - mean) ** 2).mean(dim=1))  # 标准差计算 (B, H, W)

    # 采用 2D 高斯平滑（用 PyTorch 实现）
    kernel = torch.exp(-torch.linspace(-1, 1, steps=kernel_size) ** 2 / (2 * sigma ** 2))
    kernel = kernel / kernel.sum()
    kernel = kernel.view(1, 1, -1).repeat(1, 1, 1)  # (1, 1, k)
    kernel_2d = kernel.permute(0, 1, 2).matmul(kernel.permute(0, 1, 2).transpose(2, 1))  # (1, 1, k, k)

    # 应用高斯滤波
    uncertainty = F.conv2d(uncertainty.unsqueeze(1), kernel_2d.to(uncertainty.device),
                           padding=kernel_size // 2).squeeze(1)

    return uncertainty  # (B, H, W)
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        # ✅ **将 MDFA 替换为 ETU 增强跳跃连接**
        self.enhanced_skip = EnhancedSkipConnection(head_channels)

        # 可学习参数 `alpha`
        self.alpha = nn.Parameter(torch.tensor(1.0))  # 让网络自动学习不确定性权重
        ...
        if self.config.n_skip != 0:
            ...
            for i in range(4 - self.config.n_skip):
                skip_channels[3 - i] = 0  # 重新选择 skip_channels
        else:
    ...
    def forward(self, hidden_states, features=None):
        ...
        x = self.conv_more(x)  # 变换 Transformer 输出
        ...
        for i, decoder_block in enumerate(self.blocks):
            ...
            if skip is not None:
                # 计算平滑的不确定性特征图
                U = compute_uncertainty_map(skip)
                U = U.unsqueeze(1)  # (B, 1, H, W)

                # 使用 Softmax 进行门控
                U = self.alpha * U  # 让模型自动调整不确定性影响
                W_skip, W_trans = torch.softmax(torch.cat([1 - U, U], dim=1), dim=1).split(1, dim=1)

                # ETU 增强 CNN 跳跃特征
                enhanced_skip = self.enhanced_skip(skip)

                # 结合不确定性驱动的融合策略
                fused_skip = W_skip * enhanced_skip + W_trans * x  # 低不确定性区域保留 CNN，高不确定性区域增强 Transformer

                x = decoder_block(x, skip=fused_skip)  # 进入解码器
            else:
                x = decoder_block(x)
