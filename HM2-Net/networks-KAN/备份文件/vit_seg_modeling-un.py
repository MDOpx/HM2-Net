...
class Embeddings(nn.Module):
    """ 计算 Patch Embeddings，并使用不确定性驱动的扫描调整输入顺序 """
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if config.patches.get("grid") is not None:  # Hybrid (ResNet + Transformer)
        ...
        # **不确定性扫描的排序比例**
        self.uncertainty_ratio = 0.2  # 仅 20% Patch 进行不确定性驱动排序

    def compute_uncertainty(self, x):
        """ 计算像素级通道不确定性 (标准差 STD) """
        return torch.std(x, dim=1, keepdim=True)  # 在通道维度计算标准差

    def rearrange_by_uncertainty_mixed(self, x):
        """ 部分保留原始 Patch 顺序，部分使用不确定性驱动排序 """
        uncertainty = self.compute_uncertainty(x)  # 计算每个像素的不确定性
        B, C, H, W = x.shape
        uncertainty_flat = uncertainty.view(B, -1)  # (B, H*W)

        # **获取高不确定性 Patch 的索引**
        sorted_idx = torch.argsort(uncertainty_flat, dim=-1, descending=True)  # 高不确定性优先
        num_patches = sorted_idx.shape[-1]
        num_reordered = int(self.uncertainty_ratio * num_patches)  # 仅对 20% Patch 重新排序

        # **创建新的索引，部分 Patch 按不确定性排序，部分保持原顺序**
        original_idx = torch.arange(num_reordered, num_patches, device=x.device).repeat(B, 1)  # 保持顺序
        mixed_idx = torch.cat([sorted_idx[:, :num_reordered], original_idx], dim=-1)

        # **重新排列 Patch**
        x_flat = x.view(B, C, -1)  # (B, C, H*W)
        x_sorted = torch.gather(x_flat, dim=-1, index=mixed_idx.unsqueeze(1).expand(-1, C, -1))
        return x_sorted.view(B, C, H, W)  # 变回 (B, C, H, W)
    ...
    def forward(self, x):
        ...
        x = self.patch_embeddings(x)  # 计算 Patch Embedding
        x = self.rearrange_by_uncertainty_mixed(x)  # 部分 Patch 执行不确定性扫描
        x = x.flatten(2).transpose(-1, -2)  # 变成 (B, n_patches, hidden)
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        embedding_output, features = self.embeddings(input_ids)  # 计算 Patch Embeddings + 重新排序
        encoded, attn_weights = self.encoder(embedding_output)  # 送入 Transformer 处理
