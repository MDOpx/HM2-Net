...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor, dilation=1, se_reduction=16):
        super().__init__()
        ...
        # 添加 SE 模块
        self.attn_se0 = SELayer(width, reduction=se_reduction)
        self.attn_se1 = SELayer(width * 4, reduction=se_reduction)
        self.attn_se2 = SELayer(width * 8, reduction=se_reduction)
        self.attn_se3 = SELayer(width * 16, reduction=se_reduction)
    ...
    def forward(self, x):
        ...
        for i in range(len(self.body)):
            ...
            x = getattr(self, f'attn_se{i + 1}')(x)  # 应用 SE 模块
