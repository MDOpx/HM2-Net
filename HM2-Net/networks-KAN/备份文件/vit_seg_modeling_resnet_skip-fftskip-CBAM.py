...
class CBAM(nn.Module):
    def __init__(self, in_channels, reduction=16):
        ...
        # Spatial Attention
        self.spatial_attention = SpatialAttention1()
...
class SpatialAttention1(nn.Module):
    ...
    def forward(self, x):
        ...
        x_cat = torch.cat([avg_out, max_out], 1)
...
class ResNetV2(nn.Module):
    """Implementation of Pre-activation (v2) ResNet model with SE at each stage."""

    def __init__(self, block_units, width_factor, dilation=1, cbam_reduction=16):
        super().__init__()
        ...
        # Adding CBAM after each block
        self.attn_cbam0 = CBAM(width, reduction=cbam_reduction)
        self.attn_cbam1 = CBAM(width * 4, reduction=cbam_reduction)
        self.attn_cbam2 = CBAM(width * 8, reduction=cbam_reduction)
        self.attn_cbam3 = CBAM(width * 16, reduction=cbam_reduction)
    ...
    def forward(self, x):
        ...
        features.append(x)  # 根层作为第一个跳跃连接

        x = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)(x)

        # 处理 block1, block2, block3
        for i in range(len(self.body)):
            x = self.body[i](x)  # 先经过 block
            x = getattr(self, f'attn_cbam{i + 1}')(x)  # 应用 SE 模块

            if i < 2:  # 只存储 block1 和 block2 的结果
                features.append(x)  # 存储 MDFA 后的 `x` 作为 Skip Connection

        return x, features  # 返回最后的结果和中间特征
