...
class OmniAttention(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, groups=1, reduction=0.0625, kernel_num=4, min_channel=16):
        ...
        self.channel_fc = nn.Conv2d(in_planes, in_planes, 1, bias=True)
    ...
    def get_spatial_attention(self, x):
        ...
        """
        B, C, H, W = x.shape
        x = self.avgpool(x)  # `avgpool` 可能导致 H, W 变为 1
        spatial_attention = self.spatial_fc(x)  # (B, 1, kernel_size, kernel_size)

        # 🚀 **调整 `spatial_attention` 形状**
        expected_size = (B, 1, self.kernel_size, self.kernel_size)
        if spatial_attention.shape[1:] != expected_size[1:]:
            # print(f"[DEBUG] 修正 spatial_attention 形状: {spatial_attention.shape} -> {expected_size}")
            spatial_attention = spatial_attention.view(*expected_size)

        # **🔹 使用 `F.interpolate()` 调整到 (B, 1, H, W)**
        spatial_attention = F.interpolate(spatial_attention, size=(H, W), mode="bilinear", align_corners=False)
...
from mmcv.ops import ModulatedDeformConv2d, modulated_deform_conv2d, ModulatedDeformConv2dPack
